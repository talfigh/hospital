...
class Device(models.Model):
    ...
    next_check_at = models.DateField(blank=True, verbose_name="زمان سرویس بعدی", null=True)
    hospital = models.ForeignKey(Hospital, on_delete=models.CASCADE, verbose_name="بیمارستان")

    def __str__(self):
        return f"{self.hospital} - {self.name} - {self.model}"
...
class DeviceLog(models.Model):
    ...
    name = models.CharField(max_length=64, null=True, blank=True, verbose_name="نام")
    phone_num = models.IntegerField(verbose_name="شماره تماس", blank=True, null=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"{self.device.hospital} - {self.device} > {self.created_at} "
