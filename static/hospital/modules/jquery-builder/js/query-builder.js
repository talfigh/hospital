...
QueryBuilder.prototype.getCustom = function(options) {
    options = $.extend({
        get_flags: false,
        allow_invalid: false,
        skip_empty: false
    ...
    let out = (function parse(group) {
        ...
        if (options.get_flags) {
            let flags = self.getGroupFlags(group.flags, options.get_flags === 'all');
            if (!$.isEmptyObject(flags)) {
                groupData.flags = flags;
            }
        }
        ...
        group.each(function(rule) {
            ...
            let ruleData = {};
            let ruleDetail = {};
            ruleDetail[rule.filter ? rule.filter.field : null] = value;
            ruleData[rule.operator ? rule.operator.type : null] = ruleDetail;
            ...
            if (options.get_flags) {
                let flags = self.getRuleFlags(rule.flags, options.get_flags === 'all');
                if (!$.isEmptyObject(flags)) {
                    ruleData.flags = flags;
                }
            }
    ...
    /**
     * Modifies the result of the {@link QueryBuilder#getCustom} method
     * @event changer:getCustom
     * @memberof QueryBuilder
     * @param {object} json
     * @returns {object}
     */
    return this.change('getCustom', out);
};
